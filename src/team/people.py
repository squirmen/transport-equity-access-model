...
around a cell rather than of the cell alone.
"""
...
LOCAL_BOARD = re.compile(r"(TALB|CB)\d{4}.*name|local.?board", re.IGNORECASE)
...
def build(settings: Settings, origins) -> pd.DataFrame:
    ...
    board = next((c for c in blocks.columns if LOCAL_BOARD.search(c)), None)
    fields = [c for c in BLOCK_FIELDS if c in blocks.columns] + ([board] if board else [])
    ...
    if outside.any():
        nearest = gpd.sjoin_nearest(points[points["id"].isin(joined.loc[outside, "id"])], blocks, max_distance=500)
    ...
    people = pd.DataFrame(
        {
            ...
            "local_board": joined[board].astype("string").to_numpy() if board else pd.NA,
        }
    )
